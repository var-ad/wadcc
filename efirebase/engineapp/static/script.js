...
addBtn.addEventListener("click", async () => {
    ...
    try{
      await addDoc(collection(db, "users"), { name: medicineName , dose: medicineDose , price: medicinePrice });
      loadMedicines();
    } catch (error) {
      console.log(error);
    }
  ...
  // Update Medicine by id Function
    document.querySelectorAll(".editBtn").forEach(button => {
        button.addEventListener("click", async (e) => {
            ...
            const newDose = prompt("Enter new user dose:");
            const newPrice = prompt("Enter new user price:");
            if (newName , newDose , newPrice) {
                try{
                  await updateDoc(doc(db, "users", id), { name: newName  , dose: newDose , price: newPrice });
                  loadMedicines();
